...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query("SELECT SUM(r.total) " +
            "FROM Report r JOIN City c" +
            " ON r.city.id = c.id JOIN State s" +
            " ON c.state.id = s.id " +
            "WHERE s.name = ?1" +
            " AND r.date BETWEEN ?2 AND ?3")
    ...
    @Query("SELECT new com.eugeniojava.covid19vaccination.repository.result" +
            ...
            "FROM Report r INNER JOIN Vaccine v" +
            " ON r.vaccine.id = v.id " +
            "WHERE r.date BETWEEN ?1 AND ?2 " +
            "GROUP BY v.name " +
            "ORDER BY v.id")
    List<TotalVaccineTypeResult> getTotalEachVaccineTypeByPeriod(
            LocalDate startDate,
            LocalDate endDate);
}
